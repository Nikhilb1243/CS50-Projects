...
function std(set: TextureSet, opts: THREE.MeshStandardMaterialParameters, texScale: number): WorldMaterial {
  ...
  return { material: patchFog(m, patchWet, 'world-wet'), texScale };
}
...
export function createMaterials(tex: TextureLibrary): MaterialLibrary {
  return {
    ...
    wetwood: std(tex.wetWood, { color: 0xe0d4c8, normalScale: new THREE.Vector2(1.2, 1.2) }, 2.2),
    ...
    iron: std(tex.iron, { color: 0xc8c8c8, metalness: 1 }, 2),
    ...
    flesh: std(tex.flesh, { color: 0xe8d0c8, normalScale: new THREE.Vector2(1.3, 1.3) }, 1.8),
    vertexColor: patchFog(new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 0.9, metalness: 0 }), patchWet, 'world-wet-vc'),
  };
}
