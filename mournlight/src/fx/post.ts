...
uniform float uBias;
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  // the adapted luminance is packed into RGBA8 for precision
  float l = mlUnpackLum(texture2D(uLum, vec2(0.5)));
  // gentle response: dark crypts brighten a little, never to daylight
  float e = clamp(pow(uKey / max(l, 1e-5), 0.4), uMinExp, uMaxExp) * uBias;
...
class ExposureEffect extends Effect {
  ...
  constructor() {
    super('ExposureEffect', EXPOSURE_FRAG, {
      uniforms: new Map<string, THREE.Uniform>([
        ...
        ['uKey', new THREE.Uniform(0.018)],
        ['uMinExp', new THREE.Uniform(0.7)],
        ['uMaxExp', new THREE.Uniform(1.5)],
  ...
  readLuminance(renderer: THREE.WebGLRenderer): number {
    ...
    const d = 255 / 256 / 255;
    return b[0] * d + (b[1] * d) / 256 + (b[2] * d) / 65536 + b[3] / 255 / 16777216;
  }
